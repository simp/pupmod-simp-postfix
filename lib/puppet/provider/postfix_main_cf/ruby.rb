Puppet::Type.type(:postfix_main_cf).provide(:ruby) do
  ...
  def fix_inet_interfaces
    return if Facter.value('ipv6_enabled')
    begin
      postconf('inet_interfaces')
    rescue Puppet::ExecutionFailure => error
      if error.to_s.include?('::1')
  ...
  def value
    ...
    raise(Puppet::Error, "'#{@resource[:name]}' is not recognized by postconf.") unless conf_check =~ %r{.*=\s*(.*)$}
    retval = Regexp.last_match(1).chomp
